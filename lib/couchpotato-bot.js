...
let MessageHandler = require('./handlers');
...
class CouchpotatoBot extends Bot {
		...
		this.cmds = {
			...
			direct: [
				...
				},
				{
					match: /:popcorn:/ig,
					handler: function (message, callback) {
						callback('POPCORN! omnomnomnomnom..');
					}
				}
			]
		};
	...
	_onMessage(message) {
		...
		if (this._isChatMessage(message) && !this._isFromBot(message) && this._isChannelMessage(message)) {
			...
			let user = this._getUserById(message.user).name;
		...
		if (handler !== null) {
			handler(matchResult, (response, channel, params) => {
				...
				if (target.channel) {
					...
					this.postMessageToUser(target.user, response, messageParams);
				}
			});
		}
	}
